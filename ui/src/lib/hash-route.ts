...
export interface HashRoute {
  view: "live" | "explore" | "story" | "overview" | "canvas" | "ask" | "heatmap" | "lab" | "users" | "admin";
  sessionId?: string;
...
const VALID_VIEWS = new Set(["live", "explore", "story", "overview", "canvas", "ask", "heatmap", "lab", "users", "admin"]);
...
export function parseHash(hash: string): HashRoute {
  ...
  const view = VALID_VIEWS.has(parts[0] ?? "")
    ? (parts[0] as "live" | "explore" | "story" | "overview" | "canvas" | "ask" | "heatmap" | "lab" | "users" | "admin")
    : "live";

  if (view === "users" || view === "admin" || view === "canvas" || view === "ask" || view === "heatmap" || view === "lab") {
    return { view };
  }
