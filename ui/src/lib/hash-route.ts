...
export interface HashRoute {
  view: "live" | "explore" | "story" | "overview" | "canvas" | "ask" | "users" | "admin";
  sessionId?: string;
...
const VALID_VIEWS = new Set(["live", "explore", "story", "overview", "canvas", "ask", "users", "admin"]);
...
export function parseHash(hash: string): HashRoute {
  ...
  const view = VALID_VIEWS.has(parts[0] ?? "")
    ? (parts[0] as "live" | "explore" | "story" | "overview" | "canvas" | "ask" | "users" | "admin")
    : "live";
