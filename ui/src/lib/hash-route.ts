...
export interface HashRoute {
  view: "live" | "explore" | "story" | "users" | "shapes";
  sessionId?: string;
...
const VALID_VIEWS = new Set(["live", "explore", "story", "users", "shapes"]);
...
export function parseHash(hash: string): HashRoute {
  ...
  const view = VALID_VIEWS.has(parts[0] ?? "")
    ? (parts[0] as "live" | "explore" | "story" | "users" | "shapes")
    : "live";

  if (view === "users" || view === "shapes") {
    return { view };
  }
