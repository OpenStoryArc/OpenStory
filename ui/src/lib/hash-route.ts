...
export interface HashRoute {
  view: "live" | "explore" | "story" | "canvas" | "ask" | "users" | "admin" | "fleet" | "reels" | "draw";
  sessionId?: string;
...
const VALID_VIEWS = new Set(["live", "explore", "story", "canvas", "ask", "users", "admin", "fleet", "reels", "draw"]);
...
export function parseHash(hash: string): HashRoute {
  ...
  const view = VALID_VIEWS.has(parts[0] ?? "")
    ? (parts[0] as "live" | "explore" | "story" | "canvas" | "ask" | "users" | "admin" | "fleet" | "reels" | "draw")
    : "live";
