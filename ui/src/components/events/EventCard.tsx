...
export function EventCardRow({ row, compact = false, selected = false, onClick, pairedEventId }: EventCardRowProps) {
  ...
  return (
    <div
      className={`mx-3 my-1 rounded-xl border border-[color:var(--bg-hover)] overflow-hidden hover:border-[#414868] ${onClick ? "cursor-pointer" : ""}${selected ? " ring-1 ring-[color:var(--accent)]" : ""}`}
      onClick={onClick}
    >
