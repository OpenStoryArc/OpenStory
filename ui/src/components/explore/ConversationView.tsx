...
export function ConversationView({ sessionId }: ConversationViewProps) {
  ...
  if (loading) {
    return <div className="p-4 text-xs text-[color:var(--text-muted)]">Loading conversation...</div>;
  }

  if (turns.length === 0) {
    return <div className="p-4 text-xs text-[color:var(--text-muted)]">No conversation data</div>;
  }
  ...
  return (
    <div className="p-3 space-y-3" data-testid="conversation-view">
      <div className="text-[10px] text-[color:var(--text-muted)] px-1">
