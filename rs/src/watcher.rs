...
mod tests {
    ...
    /// Guards against parallel tests mutating the shared
    /// `OPEN_STORY_WATCHER` env var. `cargo test` runs tests in parallel
    /// by default; serializing these prevents flakes.
    ...
    #[test]
    fn poll_interval_defaults_to_2000ms_and_honors_env() {
        let _g = watcher_env_lock();
        unsafe { std::env::remove_var("OPEN_STORY_POLL_INTERVAL_MS") };
        assert_eq!(poll_interval(), Duration::from_millis(2000));
        unsafe { std::env::set_var("OPEN_STORY_POLL_INTERVAL_MS", "500") };
        assert_eq!(poll_interval(), Duration::from_millis(500));
        // Garbage input falls back to the default rather than panicking.
        unsafe { std::env::set_var("OPEN_STORY_POLL_INTERVAL_MS", "fast") };
        assert_eq!(poll_interval(), Duration::from_millis(2000));
        unsafe { std::env::remove_var("OPEN_STORY_POLL_INTERVAL_MS") };
    }
