...
async fn run_hook_throughput_test(
    ...
    eprintln!(
        "\n  ══ Hooks: {rate}/s × {duration_secs}s = {total} total on {} ══",
        tier.name
    );
    ...
    print_stats(
        &format!("{} — {rate} hooks/s × {duration_secs}s", tier.name),
        &stats,
        elapsed,
    );
...
async fn perf_hooks_sustained_medium() {
    ...
    assert!(
        stats.p95_ms < 500.0,
        "p95 < 500ms on medium tier, got {:.1}ms",
        stats.p95_ms
    );
}
...
async fn perf_hooks_sustained_large() {
    ...
    assert!(
        error_rate < 0.01,
        "error rate < 1% on large tier, got {:.1}%",
        error_rate * 100.0
    );
    assert!(
        stats.p95_ms < 2000.0,
        "p95 < 2s on large tier, got {:.1}ms",
        stats.p95_ms
    );
}
