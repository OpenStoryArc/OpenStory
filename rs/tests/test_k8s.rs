...
async fn k8s_cluster_boots() {
    let cluster = K3sCluster::start().await.expect("K3s cluster should start");
    ...
    assert!(cluster.api_port() > 0, "API port should be mapped");
    ...
    assert!(
        ns_names.contains(&"default".to_string()),
        "should have default namespace"
    );
    assert!(
        ns_names.contains(&"kube-system".to_string()),
        "should have kube-system namespace"
    );
