...
mod when_publish_fails {
    ...
    fn it_logs_subject_and_error() {
        ...
        publish_failed(
            "grok",
            "events.Maxs-Air.proj.sess-1.main",
            "sess-1",
            100,
            &err,
        );
        ...
        assert!(
            ...
            "outer context kept: {error}"
        );
        assert!(
            error.contains("maximum payload exceeded"),
            "root cause kept, never dropped: {error}"
        );
    }
}
...
mod when_health_is_read {
    ...
    async fn it_reports_publish_failures() {
        ...
        assert!(
            body["publish_failures"].is_u64(),
            "a count, even when zero: {body}"
        );
    }
}
