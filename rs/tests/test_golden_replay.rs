...
// ── Codex golden replays ──
//
// Real codex rollout sessions generated by the agent-harness lab, one per
// scenario (see `tests/fixtures/codex/PROVENANCE.md`). These freeze codex's
// full type transformation — rollout JSONL → translate_codex (incl. the
// synthesized `system.turn.complete` boundary) → four-actor pipeline →
// broadcast — at parity with the pi-mono / claude golden replays above. The
// `broadcast_messages` in each snapshot are exactly what the UI receives, so a
// passing snapshot is evidence the codex path reaches the view with parity.

/// Single eval→apply→eval cycle: one tool call + result, then an answer.
#[tokio::test]
async fn golden_codex_scenario_01_single_tool() {
    let actual = capture_snapshot(
        &fixtures_root().join("codex/scenario_01_single_tool.jsonl"),
        "codex_scenario_01_single_tool",
    )
    .await;
    assert_or_update_snapshot("codex_scenario_01_single_tool", &actual);
}

/// Find-bug / fix / verify loop: multi-turn, repeated tool use, reasoning,
/// and several synthesized turn.complete boundaries.
#[tokio::test]
async fn golden_codex_scenario_02_multi_step() {
    let actual = capture_snapshot(
        &fixtures_root().join("codex/scenario_02_multi_step.jsonl"),
        "codex_scenario_02_multi_step",
    )
    .await;
    assert_or_update_snapshot("codex_scenario_02_multi_step", &actual);
}

/// Parallel tool dispatch: multiple tool calls before their results.
#[tokio::test]
async fn golden_codex_scenario_03_parallel_read() {
    let actual = capture_snapshot(
        &fixtures_root().join("codex/scenario_03_parallel_read.jsonl"),
        "codex_scenario_03_parallel_read",
    )
    .await;
    assert_or_update_snapshot("codex_scenario_03_parallel_read", &actual);
}

/// Error recovery: tool error fed back as data, then a retry.
#[tokio::test]
async fn golden_codex_scenario_04_error_recovery() {
    let actual = capture_snapshot(
        &fixtures_root().join("codex/scenario_04_error_recovery.jsonl"),
        "codex_scenario_04_error_recovery",
    )
    .await;
    assert_or_update_snapshot("codex_scenario_04_error_recovery", &actual);
}

/// Large tool output: exercises truncation / payload budgeting on the
/// tool_result path.
#[tokio::test]
async fn golden_codex_scenario_05_big_output() {
    let actual = capture_snapshot(
        &fixtures_root().join("codex/scenario_05_big_output.jsonl"),
        "codex_scenario_05_big_output",
    )
    .await;
