...
mod when_replay_is_running {
    ...
    async fn it_reports_phase_and_progress() {
        ...
        assert_eq!(body["boot"]["replay"]["done"], 20, "done equals total once serving");
    }
}
...
mod when_replaying {
    ...
    async fn it_returns_503_for_readiness_and_200_for_liveness() {
        ...
        let ready = send_request(state.clone(), Request::get("/api/health").body(Body::empty()).unwrap()).await;
        assert_eq!(ready.status(), 503, "not ready while replaying");
        let body = body_json(ready).await;
        assert_eq!(body["boot"]["phase"], "replaying", "the body still explains itself");

        let live = send_request(state.clone(), Request::get("/health").body(Body::empty()).unwrap()).await;
        assert_eq!(live.status(), 200, "alive is alive");

        boot::set_serving();
        let ready = send_request(state, Request::get("/api/health").body(Body::empty()).unwrap()).await;
