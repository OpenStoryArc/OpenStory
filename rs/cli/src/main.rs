        ...
        None | Some(Command::Serve { .. }) => {
            let (cli_overrides, static_dir) = match cli.command {
                Some(Command::Serve {
                    ...
                    manage_nats, nats_bin,
                }) => ((role, host, port, data_dir, watch_dir, nats_url,
                        max_initial_records, watch_backfill_hours, truncation_threshold,
                        stale_threshold_secs, api_token, db_key, metrics,
                        data_backend, mongo_uri, mongo_db, init_config,
                        manage_nats, nats_bin), static_dir),
            ...
            let (cli_role, cli_host, cli_port, cli_data_dir, cli_watch_dir, cli_nats_url,
                 ...
                 init_config, cli_manage_nats, cli_nats_bin) = cli_overrides;
            ...
            // exit `_nats_guard` drops and stops any nats-server we spawned —
            // Rust runs no destructors on a bare signal kill.
            let serving = server::run_server(
                &host, port, &data_dir, static_dir.as_deref(), &watch_dir, bus, config,
            );
