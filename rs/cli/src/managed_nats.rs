...
pub fn ensure_nats(
    ...
    match leaf_url {
        Some(url) => {
            ...
            std::fs::write(
                &conf_path,
                render_leaf_config(&host, port, store_dir, url, domain),
            )
            .map_err(|e| {
                anyhow::anyhow!("cannot write leaf config {}: {e}", conf_path.display())
            })?;
            eprintln!(
        ...
        None => {
            // Loopback only, but still from a config: the payload limit must
            // match the leaf's 8 MB, or a transcript line over nats-server's
            // 1 MiB default can never be published.
            let conf_path = store_dir.join("standalone.conf");
            std::fs::write(
                &conf_path,
                render_standalone_config(&host, port, store_dir, domain),
            )
            .map_err(|e| {
                anyhow::anyhow!(
                    "cannot write standalone config {}: {e}",
                    conf_path.display()
                )
            })?;
            eprintln!(
...
/// The standalone (loopback-only) server's config: the leaf template without
/// the remote, so the JetStream store and the 8 MB payload limit are the
/// same whether or not a node federates.
/// The JetStream domain this node's managed NATS must serve (F-02), the
/// one the bus pins: the hub domain on a hub (consumer role with
/// `OPEN_STORY_HUB_DOMAIN`), this host on a leaf (`OPEN_STORY_HUB_DOMAIN`
/// on any other role) or a mesh device (`OPEN_STORY_PEER_DOMAINS`), and
/// none solo, so a single-machine install renders exactly what it did.
/// Pure.
pub fn local_domain(
    ...
    match hub_domain.map(str::trim).filter(|d| !d.is_empty()) {
        Some(d) if is_hub => Some(d.to_string()),
        Some(_) => Some(host.to_string()),
        None if mesh => Some(host.to_string()),
        None => None,
    }
}
...
pub fn render_standalone_config(
    ...
    format!(
        "# Generated by `open-story serve` — managed JetStream, loopback only.\n\
         # Set nats_leaf_url to federate; this file then becomes leaf.conf.\n\
         listen: {host}:{port}\n\
         \n\
         jetstream {{\n    \
             store_dir: \"{store}\"\n    \
...
mod tests {
    ...
    fn standalone_config_raises_max_payload_like_the_leaf() {
        let conf = render_standalone_config("127.0.0.1", 4322, Path::new("/tmp/store"), None);
        assert!(conf.contains("listen: 127.0.0.1:4322"), "{conf}");
        assert!(
            conf.contains("max_payload: 8MB"),
            "the leaf's limit: {conf}"
        );
        assert!(conf.contains("store_dir: \"/tmp/store\""), "{conf}");
        assert!(conf.contains("jetstream {"), "{conf}");
        assert!(
            !conf.contains("leafnodes"),
            "standalone federates nowhere: {conf}"
        );
    }
    ...
    mod when_a_domain_is_given {
        ...
        fn it_renders_the_domain_in_both_shapes() {
            ...
            let solo =
                render_standalone_config("127.0.0.1", 4322, Path::new("/tmp/x"), Some("hub"));
        ...
        fn it_is_the_hub_domain_on_a_hub_and_the_host_on_a_leaf() {
            ...
            assert_eq!(
                local_domain(Some("hub"), true, false, "maxs-air").as_deref(),
                Some("hub")
            );
            assert_eq!(
                local_domain(Some("hub"), false, false, "maxs-air").as_deref(),
                Some("maxs-air")
            );
            assert_eq!(
                local_domain(None, false, true, "maxs-air").as_deref(),
                Some("maxs-air")
            );
