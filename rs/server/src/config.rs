...
pub struct Config {
    ...
    /// Path to a nats-server conf file generated by `AccountConfigWriter`
    /// at boot. When set AND `[person]` is configured, the server writes a
    /// per-person NATS *account* block here so the operator can run
    /// nats-server with hard account isolation (`nats-server -c <path>`) —
    /// a real, NATS-enforced partition where each person's principals share
    /// one account. This is an advanced, opt-in deployment shape; most
    /// nodes use a single shared account + token (see `docs/deploy/`).
    /// **Warning:** the writer owns the file — don't point it at a
    /// hand-managed `nats.conf`. Empty = single-account mode (the default).
    /// NOTE: cross-account sharing is NOT dynamically managed — there is no
    /// per-session "share with person" anymore; what a node publishes is
    /// governed by `publish_sessions` and the network it joins, not by
    /// runtime account grants.
    #[serde(default)]
...
impl Config {
    ...
    /// single-user trusted-local box and never auto-grants Admin.
