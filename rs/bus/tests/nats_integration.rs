...
/// Phase 4.10 — bus-level Invariant ① enforcement.
///
/// Mark a session as `private` via the lookup, publish a batch for that
/// session, assert: (a) publish returns Ok with no jetstream error,
/// (b) the bus's skip counter incremented, (c) a subscriber NEVER
/// receives the event on the NATS subject. Together these prove the
/// event truly stayed local — the bus refused to carry what the
/// operator marked private.
#[tokio::test]
#[ignore]
async fn publish_skips_private_session() {
    use async_trait::async_trait;
    use open_story_bus::share_policy::SharePolicyLookup;
    use std::sync::Arc;

    struct AllPrivateForSession(String);
    #[async_trait]
    impl SharePolicyLookup for AllPrivateForSession {
        async fn is_private(&self, session_id: &str) -> bool {
            session_id == self.0
        }
    }

    let private_session = format!("private-{}", uuid::Uuid::new_v4());
    let public_session = format!("public-{}", uuid::Uuid::new_v4());

    let bus = NatsBus::connect(&nats_url())
        .await
        .expect("connect")
        .with_policy_lookup(Arc::new(AllPrivateForSession(private_session.clone())));
    bus.ensure_streams().await.expect("ensure streams");

    let mut sub = bus.subscribe("events.>").await.expect("subscribe");

    // First: publish for the PRIVATE session — must be skipped.
    let private_batch = IngestBatch {
        session_id: private_session.clone(),
        project_id: "test-project".to_string(),
        events: vec![test_event("private-event")],
    };
    bus.publish(&format!("events.session.{private_session}"), &private_batch)
        .await
        .expect("publish (skipped path is still Ok)");
    assert_eq!(
        bus.publish_skipped_private_count(),
        1,
        "skip counter must have incremented"
    );

    // Now publish for the PUBLIC session — must arrive on the bus.
    let public_batch = IngestBatch {
        session_id: public_session.clone(),
        project_id: "test-project".to_string(),
        events: vec![test_event("public-event")],
    };
    bus.publish(&format!("events.session.{public_session}"), &public_batch)
        .await
        .expect("publish");
    assert_eq!(
        bus.publish_skipped_private_count(),
        1,
        "public publish must not increment the skip counter"
    );

    // The subscriber should receive ONLY the public session — within a
    // bounded window. The private one never enters NATS.
    let received = tokio::time::timeout(Duration::from_secs(5), sub.receiver.recv())
        .await
        .expect("timeout waiting for public batch")
        .expect("channel closed");
    assert_eq!(
        received.session_id, public_session,
        "subscriber must receive ONLY the public batch — the private one was skipped at publish"
    );

    // And nothing else within a short window — proves the private
    // wasn't merely delayed.
    let extra = tokio::time::timeout(Duration::from_millis(500), sub.receiver.recv()).await;
    assert!(
        extra.is_err(),
        "no further batches expected — the private one must have been skipped, not just delayed"
    );
}
