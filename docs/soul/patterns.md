...
The failure mode is the worst kind: silent. The container boots cleanly, `/api/sessions` returns 200, the UI loads. The only symptom is that data gets *frozen in time* — new sessions appear only after a container restart (boot backfill), and live sessions never update. No errors, no warnings, no log lines. Just a watcher waiting for events that the kernel will never deliver because the kernel never received them in the first place. Watch-dir-only sources (Cowork, pi-mono, Hermes) are hit hardest; Claude Code CLI sessions partially mask the bug because HTTP hooks deliver events independently of the file watcher.
