...
Last updated: 2026-09-24 (H group complete).
...
| H · health | H-01 … H-08 | 8 / 8 | `/api/health` can say no |
...
| H-07 | GREEN. `/api/health` includes `version`, `git_sha`, `built_at`, `data_dir`, `store.size_bytes`, `process.rss_bytes`, `uptime_secs`. | `…::when_health_is_read::it_stamps_version_and_sha` |
| H-08 | GREEN. The dashboard header shows a dot: green when health is ok, amber on any warn, red on any critical, with the JSON one click away. | `ui/tests/components/health-dot.test.tsx::when_health_has_a_critical::it_shows_red_with_the_reason` |
...
| P-02 | GREEN. Presence is an observed family: the persist consumer stores it in its own table (`presence`), never in `events`. | `…::when_presence_arrives::it_lands_in_the_presence_table_not_events` |
...
| O-04 | GREEN. The `observe/` stack (Prometheus and Grafana under `just observe`) gets one dashboard, "Node", with the O-01 gauges; the 2026-03 dashboards are removed or updated. | `scripts/check_docs.py` gains a check that dashboard panel queries reference existing metric names |
| O-05 | OWNER. Reviewer gate; the loop does not close PRs. PR #46 is closed with a comment pointing at this work; nothing from it is merged. | reviewer gate |
...
| id | requirement | acceptance test |
|---|---|---|
| K-01 | GREEN (run on a1 over ssh, 2026-09-24 01:41 local, with the published `ghcr.io/openstoryarc/open-story:latest`: namespace created, PVCs provisioned, rollout available, the probe read the pod and its sidecar with verdict ok, namespace deleted; the branch's own image needs a registry push, which is the owner's). `deploy/k8s/` holds a kustomize base: Deployment (server + NATS leaf sidecar), two PVCs (store, jetstream), ConfigMap from `config.toml`, Secret for the leaf URL, Service on 3002, and a `os-loop-` namespace overlay for a1. `scripts/k3s_smoke.sh` applies it on a1, waits for ready, runs the probe against the pod, and tears the namespace down. | `scripts/k3s_smoke.sh --test` (dry run) and one real run on a1 recorded in the design doc |
...
| K-04 | PARTIAL (the manifest sets `OPEN_STORY_LOG_FORMAT=json`, checked; the a1 run showed text lines because the published image predates L-01; it flips once the branch image is pushed and the smoke re-run). Logs go to stdout in JSON (L-01); `kubectl logs` shows one JSON object per line. | recorded in the a1 run |
...
| id | requirement | acceptance test |
|---|---|---|
| D-01 | GREEN. Every log line and `/api/health` carry `git_sha` and `built_at` (H-07), so a change is identifiable in every signal. | covered by H-07 and L-02 |
...
| D-05 | PARTIAL (documented for brew, compose, k3s in `docs/deploy/operations.md`; none exercised end to end yet, each says so). Rollback is a documented one-liner per host shape (brew, compose, k3s) in `docs/deploy/operations.md`, verified once on a1. | doc plus the a1 run |
...
- **2026-09-23 23:38 local.** H-07 GREEN. `rs/server/build.rs` stamps
  ...
  `logging::init` at boot), `store_size_bytes` (walkdir over the data dir),
...
- **2026-09-24 00:12 local.** P-01 to P-04 GREEN. The node beats every
  ...
  both backends (conformance helper 64) and a supervised `presence` actor
  reads `presence.>`; `GET /api/fleet/presence` gives each node
  `age_secs` and `stale` past three beats; the bus federates presence
  like events (per-host stream, mirror, aggregate, grants). Corrections
  this hour: the loop log's timestamps had drifted four hours ahead of
  the clock and were rewritten from commit times. Owner must decide:
  `openstory-deploy` runs the hub, so its next deploy creates
