...
### Plan-file watcher (`~/.claude/plans/`)
Plans land in OpenStory two ways today: as `ExitPlanMode` content inside the transcript stream (which **Plan Visibility** above wants to surface in the UI), and as files on disk under `~/.claude/plans/*.md` — which OpenStory does not watch at all. The file form is the *persistent, editable* substrate of intent: the plan you reread when execution drifts, mark up after a run, return to a week later. Treating these files as a watch source would surface plan evolution, executed-vs-abandoned plans, and the gap between intent and result. Concrete shape: add a `plans_watch_dir` config field (default `~/.claude/plans/`), translate `.md` writes into CloudEvents (subtype `intent.plan.update` or similar), and link to sessions where possible — by mtime proximity to session activity, or by content match (the plan filename often appears in transcript prompts). Distinct from in-transcript `ExitPlanMode` events; the two share a UI affordance with **Plan Visibility** once both exist. Motivated by the observation that every plan we've written for OpenStory itself lives on disk and is invisible to OpenStory's own listener.

### Harness source observation (research spike)
OpenStory watches the *output* of agent harnesses (Claude Code, pi-mono, Hermes — transcripts, hooks, NATS events) but not their *source*. The harness source is the ground truth for what events can exist: new tools, schema changes, hook renames, model bindings. When the harness moves, OpenStory's translators in `rs/core/src/translate*.rs` can silently fall behind — a tool name changes, an event field is added, and translation degrades to a fallback path with no signal that something changed. A research spike could: watch the source repos / npm packages / release notes of the three harnesses; extract structural facts (tool definitions, event/hook schemas, model strings); cross-reference with the local translators and consumers to surface drift ("Claude Code added tool X; your translator doesn't know about it"; "pi-mono's session schema changed; `translate_pi.rs` may need updating"). Probably starts as `docs/research/harness-drift/` — scope first, then decide whether the answer is a CI check, a startup warning, or its own consumer actor.
...
### Agent-user attribution for pi-mono sessions (the "Bobby gap")
Surfaced 2026-05-16 during the retrospective writeup. `/api/sessions` returns a `user` field for path-attributable sessions (e.g., `maxglassie`, `katie`) but pi-mono sessions arrive via NATS with `pi://session/...` source URIs and no `/Users/X/` paths, so they fall through to `<unattributed>`. Consequence: **all 5 pi-mono sessions in the corpus (~6,500 events including Bobby's 6,394-event primary session) are invisible to every shape-layer aggregate that filters by user**. The cache_hit_rate, verb posture, attention pattern, PR cadence — none of these include Bobby's signature. Fix candidates: (a) extend the CloudEvent translator to stamp a `user: "bobby"` (or `user: "agent:pi-mono:{instance_id}"`) attribute when `agent: "pi-mono"`; (b) extend the session-list projection to derive `user` from `agent` + a config-mapping for known pi-mono deployments; (c) add a separate `harness` field at the session-list level so consumers can filter by `harness IN ('claude-code', 'pi-mono', 'hermes')` directly. Option (b) is the smallest change. Once stamped, re-run `scripts/build_*_shapes.py` and the retrospective metrics widen to include Bobby. See `personal/reports/openstory-retrospective-TRACEABILITY.md` §6c for the verified evidence and `scripts/analyze_pi_mono.py` (this commit) for an interim harness-based analysis pipeline.

### Diffable feedback-run schema
The session-stories methodology currently outputs prose (HTML/PPTX/markdown). Two runs of the same methodology aren't structurally comparable — diffs are text diffs, not claim-level diffs. Design a schema where each conclusion is an addressable atom with a stable id, declarative interpretation rules, and a numeric metric body. Two runs produce two YAML/JSON snapshots; a diff tool produces a third document identifying notable changes (label flips, threshold crossings, ≥20% metric deltas). Enables: time-series of any individual claim, cross-person comparison via `window.scope`, alerting on label flips ("`horizon.architectural.healthcheck` flipped from watching → incubating this week"), and auditability of every conclusion against the underlying queries. Three artifacts needed: `personal/feedback-runs/schema.json` (JSON Schema), `scripts/build_feedback_run.py` (one fn per claim), `scripts/diff_feedback_runs.py` (delta detection). Renderer becomes a separate concern (`render_feedback_run.py`) — prose moves from the artifact to the template layer. See `personal/feedback-slides/TRACEABILITY.md` for the current prose-shaped version.
