...
### Gate Docker integration tests behind `#[ignore]` so `cargo test` is fast by default
Surfaced 2026-06-11 profiling the dev test loop. Plain `cargo test` runs **~372s** — and ~360s of that is **7 Docker-dependent integration binaries** that run inline with no gate: `test_hermes_translator` (108.9s — boots a `hermes-fixture:test` image that isn't built locally, so testcontainers burns ~100s on pull/retry), `test_pi_mono_storytelling` (60.5s), `test_convergence_invariants` (60.4s), `test_split_deployment` (60.4s — the 60s ones are timeouts waiting on Docker/NATS that the inner-loop dev hasn't set up), `test_pi_mono_container` (36.9s), `test_projection_shared_map` (18.6s), `test_compose_perf` (14.6s). The other 55 binaries + all 699 unit tests run in ~13s cold / **~0.9s warm**. Compile/link is NOT the bottleneck (0.15s warm, ~8s after touching a core lib) — it's pure Docker execution/timeout. `just test`'s existing `-- --skip compose --skip container` only catches 2 of the 7 (it's a test-*name* filter, and the worst offenders' names don't contain those substrings), so even `just test` is ~5min. **Immediate relief shipped:** `just test-fast` (`cargo test --workspace --exclude open-story-cli --lib`) for the edit-loop. **Structural fix (this entry):** add `#[ignore = "docker; run via just test-<x>"]` to the ~7 Docker integration binaries (or a shared `requires_docker!()` gate macro), so plain `cargo test` skips them by default and they run via the existing `test-container`/`test-compose`/etc. recipes (which already `docker-build` first) plus a CI step using `--include-ignored`. Net: `cargo test` drops from ~6min to ~13s with zero coverage loss — the Docker tests still run, just explicitly. Also fix the `test_hermes_translator` pull-retry: it should fail fast (or skip) when `hermes-fixture:test` is absent instead of burning 100s. Consider `cargo-nextest` as a complementary win (parallelizes across binaries) but it won't fix the timeouts — `#[ignore]` is the primary lever.
