...
**Scope boundary — chat tab is structurally unobservable.** The desktop app is two different products in one window. The Code tab (Cowork) is local-first: full JSONL transcript on disk, observable by this integration. The Chat tab is a webview of claude.ai: conversations live server-side, and the only local trace is Chromium renderer cache (`IndexedDB/https_claude.ai_0.indexeddb.leveldb` — ephemeral, partial, undocumented). No file exists for the watcher to watch, so chat sessions will never appear via this path. Verified empirically 2026-06-09: zero JSONL outside `local-agent-mode-sessions/`. If chat visibility ever matters, the candidates are the OTel exporter (see RESEARCH entry below) or claude.ai's bulk data export as a one-shot backfill — not the filesystem.
...
A Cowork session has *two* UUIDs: the outer "task" UUID (Cowork-side) and the inner Claude Code `session_id` in the JSONL. They're distinct (e.g., task `4d7a35aa-…`, inner session `65dc744d-…`). The path-encoded project name is also pathological — it contains every parent UUID, producing a `project_id` like `-Users-kloughra-Library-Application-Support-Claude-local-agent-mode-sessions-89bbf81e-…-4d7a35aa-…-local-4c6d830b-…-output-e6evvn`. Decide whether to collapse these into a single virtual "Cowork" project group in the sidebar, or to surface them as-is. **Update (2026-06-09, live test):** the watcher currently extracts the *account UUID* as `project_id`, so all Cowork sessions group under one meaningless UUID header. But the task dir also contains **`spaces.json`** — Claude Desktop's registry of Projects ("spaces" internally), with `id`, human `name` (e.g. `"test-project"`), and `folders[].path`. Desktop-Project-scoped sessions write the same JSONL as standalone ones; `spaces.json` is the linkage for labeling groups with real project names instead of inventing a grouping heuristic. ~40 LOC in the UI sidebar reducer + a small `spaces.json` reader. Estimate: half a day.
...
### PollWatcher fallback for VirtioFS-mounted watch dirs
Found during the 2026-06-09 live test: in a Docker deployment on macOS, **Cowork ingestion is backfill-only** — new sessions appear only on container restart. Root cause: `watch_with_callback` uses `RecommendedWatcher` (inotify inside Linux containers) unconditionally, and Docker Desktop's VirtioFS bind mounts do not propagate inotify events for host-side writes. The regular `watch_dir` masks this because Claude Code CLI sessions also arrive via HTTP hooks (`~/.claude/settings.json` → `POST /hooks`); the Cowork VM has no hooks pointing at the host, so the file watcher is its *only* live path. Fix: detect when a watch dir gets no inotify events (or detect containerized-macOS via heuristic/config flag) and fall back to `notify::PollWatcher` with an mtime-based poll interval. **Prior art exists:** commit `ca32270` (unmerged branch `fix/sprint1-cloudevent-typed-followup`, 2026-04-27) contains a complete implementation — `WatcherKind::from_env()` honoring `OPEN_STORY_WATCHER` with `/.dockerenv` auto-detection, `OPEN_STORY_POLL_INTERVAL_MS` tunable (default 2000ms), and a `build_watcher()` helper that logs the chosen backend. Salvage just the `rs/src/watcher.rs` + Dockerfile env + `docs/soul/patterns.md` hunks; the WIP commit bundles ~1000 lines of unrelated research docs that should not ride along. Note `rs/tests/test_hermes_snapshot_watcher.rs:182` also anticipated this ("if it fails, PollWatcher is needed instead"). Affects pi/hermes/cowork dirs equally in Docker; native installs (brew, FSEvents) are unaffected. Estimate: half a day including extraction + a container test.
