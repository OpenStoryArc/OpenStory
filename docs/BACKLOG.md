...
Letting OpenStory observe Hermes Agent (NousResearch/hermes-agent) sessions. The Rust translator is shipped (`rs/core/src/translate_hermes.rs`); what remains is the plumbing to get Hermes artifacts into the watcher pipeline and the downstream analytics that become possible once Hermes sessions appear alongside Claude Code and pi-mono ones. Architectural framing at [`docs/research/LISTENER_AS_ALGEBRA.md`](research/LISTENER_AS_ALGEBRA.md).

Design decision: **no plugin.** The original integration brief proposed a pip-installable Hermes plugin that would emit per-event JSONL into a watched directory. That path was rejected — it requires agent cooperation, which violates the "observe, never interfere" principle. Instead, OpenStory reads artifacts Hermes already writes for its own reasons: the per-turn snapshot at `~/.hermes/sessions/session_{id}.json` (always present) or the append-only gateway transcript at `~/.hermes/sessions/{id}.jsonl` (gateway mode only). Earlier research on the plugin path was removed in the same change window — see git history if you need the archaeology.

### Hermes session-JSON ingest adapter
A sidecar or in-process converter that reads `~/.hermes/sessions/session_{id}.json` snapshots, diffs against the previous state, and emits any new messages as envelope-format JSONL into a watched directory. Fixture `rs/tests/fixtures/hermes/Dockerfile.hermes` has a reference `convert_session.py` that does the conversion for test sessions — lift it into production. Alternative: a snapshot-diff watcher mode in `reader.rs` that natively handles whole-file rewrites (more code, avoids the intermediate file). Estimate: 1 day for the sidecar approach. A half-day more for a compose template that deploys Hermes alongside Bobby/Vera on the VPS.

### Gateway-mode direct ingest
When Hermes runs in gateway mode, it writes an append-only `~/.hermes/sessions/{id}.jsonl`. This matches OpenStory's existing byte-offset watcher model — a watcher mount at `/pi-watch/hermes-{name}/` plus a format sniff for the Hermes envelope shape is all that's needed. No converter required. Trade-off: CLI-mode Hermes sessions aren't captured; only gateway mode. For a VPS deployment (which is gateway-shaped), this is the simpler path than the session-JSON adapter.

### Cross-provider behavioral comparison endpoint
A new `GET /api/insights/provider-comparison` endpoint that aggregates structural metrics per provider for the same task: cycles per task, error rates, tool selections, terminal stop reasons. Hermes is provider-polymorphic and already supports running the same task across providers via `batch_runner.py`; OpenStory's job is the aggregation and the view. The most novel research output of the integration.

### StructuralTurn training data export
A new `GET /api/sessions/{id}/training-export?format=structural-jsonl` endpoint that emits `StructuralTurn`s (with eval/apply phases separated, domain facts extracted, subagent boundaries explicit, `ToolOutcome` typed) as training data. Lets Hermes's trajectory pipeline (`trajectory_compressor.py`, the tinker-atropos integration) consume the structurally-decomposed view alongside or instead of raw messages. Open research question: does training on structurally-decomposed traces produce better tool-calling models? The two repos are uniquely positioned to answer it. Depends on at least one ingest path being wired up first.
