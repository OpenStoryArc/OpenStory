...
Found during the 2026-06-09 live test: in a Docker deployment on macOS, **Cowork ingestion is backfill-only** — new sessions appear only on container restart. Root cause: `watch_with_callback` uses `RecommendedWatcher` (inotify inside Linux containers) unconditionally, and Docker Desktop's VirtioFS bind mounts do not propagate inotify events for host-side writes. The regular `watch_dir` masks this because Claude Code CLI sessions also arrive via HTTP hooks (`~/.claude/settings.json` → `POST /hooks`); the Cowork VM has no hooks pointing at the host, so the file watcher is its *only* live path. Fix: detect when a watch dir gets no inotify events (or detect containerized-macOS via heuristic/config flag) and fall back to `notify::PollWatcher` with an mtime-based poll interval. Note `rs/tests/test_hermes_snapshot_watcher.rs:182` already anticipated this ("if it fails, PollWatcher is needed instead") — the contingency was never wired in. Affects pi/hermes/cowork dirs equally in Docker; native installs (brew, FSEvents) are unaffected. Estimate: half a day + a container test.
