...
### InsightExtraction consumer — LLM-derived structured facts
A fifth actor-consumer that subscribes to `patterns.>` (specifically `turn.sentence` patterns), buffers per session until completion, calls an LLM to extract typed structured facts (`skill_pattern`, `identified_gap`, `controllability`, `prompt_pattern`, `efficiency_loss`), and writes them to a new `session_insights` SQLite/Mongo table with evidence event_ids attached. Fills the architectural gap the DORA Advisor identifies (see `workspace/agentic-learning/dora-metrics/ARCHITECTURE.md`): deterministic pattern detection (already shipped) is *structural*; insight extraction is *semantic*. Both are valuable; OpenStory currently only has the first. **Off by default** — extracting conversation content via LLM violates the sovereignty posture unless explicitly opted in. Local-model path (Ollama-shaped) matters as much as API path. Full design with schema, trigger model, privacy boundaries, and open questions: [`docs/research/insight-extraction-consumer.md`](research/insight-extraction-consumer.md). Drives the per-user "what they're doing" surface in the Users tab — the Users tab v0.1 uses session labels as a stand-in; once this consumer ships, the same UI surface shows real extracted insights. Estimated ~600 LOC for a usable v0.1 (consumer + store + one MCP tool); ~1500 LOC end-to-end with API + UI panel.

### Users tab — per-user activity surface (v0.1 → v1)
A fourth top-level tab alongside Live / Story / Explore. v0.1 (in flight): aggregates `SessionRow` rows by the `user` field added in PR #42, renders a card per user with session count, hosts, projects, recent sessions, last activity, token totals. Click a user → navigates to existing `?user=X` filter in the Live tab. "What they're doing" starts as the most recent session's label (deterministic, LLM-free). v1: once the InsightExtraction consumer above ships, the same Users tab UI swaps the label-based summary for structured insights (skill patterns, gaps, prompt patterns) — UI surface stays the same, the data feed changes. Backend endpoint: `GET /api/users` returning `{ users: [...], total }`. Estimated ~250 LOC for v0.1.

### Multi-source event bus — git, CI, incidents alongside agent transcripts (DORA-pattern adaptation)
Today OpenStory's bus carries one signal class: agent transcripts (Claude Code, pi-mono, eventually Hermes). DORA carries five — git commits, CI/CD pipelines, incidents, coding sessions, chat messages — all under the same CloudEvents envelope. Subject hierarchy `events.{project}.{session}.>` could expand to `events.{project}.{source}.{session}.>` where `source ∈ {claude-code, pi-mono, git, ci, incident, ...}`. Each source gets its own translator. Bus is ready; consumers + views layer would handle non-message subtypes. The interesting unlock: cross-source correlation — "every time CI failed last week, what was the agent doing 10 min prior?" Per-source, one at a time, like the Hermes integration is shaping up.

### `/api/ask` — multi-store agentic advisor (DORA-pattern adaptation)
Today OpenStory exposes ~25 MCP tools and the *agent* orchestrates: pick which to call, combine results, synthesize. The DORA Advisor inverts this — server-side `/api/ask` orchestrates across stores (analytics queries + FTS5 + future vector store + future `session_insights`) and an LLM synthesizes. Example: "Why did my context fill so fast this session?" → query `token_usage` → search FTS for tool repeats → check `session_insights` for known efficiency-loss patterns → narrate. Both shapes are valid; this is the friendlier path for direct user questions. Builds on the existing analytics + the future InsightExtraction consumer. Auth + cost controls are the only meaningful new surface.

### Re-introduce vector store as architectural peer (semantic search beyond keywords)
The `rs/semantic/` crate is currently slated for removal (see "Remove Orphaned Semantic Crate"). FTS5 covers keyword search well; it does not cover semantic similarity ("find sessions where I was debugging something similar to today"). DORA treats the vector store as a peer to the structured DB, populated by an async embedding worker on session completion. The cleaner path is not to revive the orphaned Qdrant code but to design the vector store as a third store alongside SQLite and the future `session_insights` table. Open question: per-user collection isolation (DORA's choice) vs single project-scoped collection. Worth re-evaluating only if/when "I want to find that session about X but I don't remember the words I used" becomes a real user need — keyword-only is a reasonable holding pattern.

### Storage-layer tenant isolation for team mode
DORA's lesson: "namespace isolation over metadata filtering — one bug in a filter = a data leak." OpenStory is single-tenant today, but the multi-machine NATS leaf deployment means every node holds every team member's full data. Existing backlog item "Distributed Deployment Security Hardening" addresses *bus-level* isolation; this entry is the complementary *storage-level* lesson: once data lands locally, a `WHERE user_id = X` filter is one bug from a leak. Equivalent for OpenStory: per-user SQLite files, per-user JSONL directories, per-user vector collections — the storage layer enforces the boundary, not the application layer. Becomes load-bearing only if OpenStory pivots from "your laptop" to "shared team product"; flagged here so that pivot doesn't accidentally inherit a filter-based isolation model.
