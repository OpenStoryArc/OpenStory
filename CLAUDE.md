...
- **Don't trust inotify inside Docker on macOS.** Docker Desktop on macOS bridges the host filesystem into the Linux VM via VirtioFS (or gRPC FUSE). That bridge does **not** propagate filesystem events. A `notify::RecommendedWatcher` in a container therefore boots, looks healthy, and silently misses every write that originates on the macOS host — sessions never appear in the UI, the API's `start_time` freezes 5 days behind reality, and nothing logs an error. The fix is `notify::PollWatcher` for container builds: pay 2s detection latency and single-digit CPU% to get reliable observation. The selection is in `rs/src/watcher.rs::WatcherKind::from_env()` — `OPEN_STORY_WATCHER=poll|recommended` overrides; otherwise auto-detect via `/.dockerenv`. The Dockerfile sets `OPEN_STORY_WATCHER=poll` so `docker compose up` works on every host. Native runs (`just up-no-mongo`, `just serve`) keep the inotify/FSEvents fast path.

> **Important note on the watcher tradeoff.** Polling is the right default *only inside containers*. On native runs, inotify (Linux) and FSEvents (macOS native) are millisecond-class and effectively free; polling there would burn battery for no UX win. The two-mode design is deliberate: containers get reliability, native runs keep speed. If you change the watcher, preserve both paths and the explicit-override env var — silent regressions on either platform are exactly the trap this note exists to prevent.
