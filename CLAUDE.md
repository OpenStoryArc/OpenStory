...
- **Don't trust inotify inside Docker on macOS.** Docker Desktop bridges the host filesystem into the Linux VM via VirtioFS, which does **not** propagate filesystem events. A `notify::RecommendedWatcher` in a container boots, looks healthy, and silently misses every host-originated write — new sessions appear only on container restart, and nothing logs an error. The fix is `notify::PollWatcher` for container builds: pay 2s detection latency and single-digit CPU% for reliable observation. Selection is in `rs/src/watcher.rs::WatcherKind::from_env()` — `OPEN_STORY_WATCHER=poll|recommended` overrides; otherwise auto-detect via `/.dockerenv`. The Dockerfiles set `OPEN_STORY_WATCHER=poll`. Native runs keep the inotify/FSEvents fast path — if you change the watcher, preserve both modes and the env override.
...
When the user asks to start, run, boot, or "stand up" OpenStory:
...
**Why Vite runs separately in dev (and not in prod):** Vite is a separate process here purely for hot module reload while editing UI code. The Rust server can serve UI static files directly via `--static-dir` (the capability is in `rs/server/src/router.rs:226` and has been since the initial commit). `Dockerfile.prod` uses exactly that path — `npm run build` → `ui/dist` → `--static-dir /static` — so the deployed shape is a single binary serving both API and UI. The dev split is a UI-iteration ergonomics choice, not an architectural one.
