...
- **`scripts/check_config.py`** — config-table drift validator (sibling of `check_docs.py`). Parses the `Config` struct + `Default` impl in `rs/server/src/config.rs` and checks the CLAUDE.md Configuration table against it: flags documented fields that no longer exist (ghosts/renames) and documented defaults that disagree with the code. Has a `--test` flag.
...
When the user asks to start, run, boot, or "stand up" OpenStory, use the **`boot` skill** (`.claude/skills/boot/SKILL.md`). It carries the full flow — check-if-running → pick-mode → verify-prereqs → check-ports → boot → verify — plus the conversational tone and the three modes (Docker / native SQLite / split dev). All modes serve the UI at http://localhost:5173 and the API at http://localhost:3002.
